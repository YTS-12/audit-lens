#!/bin/bash
# ══════════════════════════════════════════════════════════════════
# 감사렌즈 완전 무인 스케줄 설정 (수정본 v3 — 재실행 안전)
# 사용법: AWS 콘솔(관리자) → CloudShell(>_) → 전체 붙여넣기 → Enter
# 효과: 평일 09:00 KST 자동 시작 + 21:00 자동 종료(백업). 주말은 수동 유지.
# ══════════════════════════════════════════════════════════════════
...
# ③ 평일 21:00 KST 자동 종료(서버 내부 21:00 크론의 백업) — 이미 있으면 건너뜀
aws scheduler get-schedule --region $R --name audit-lens-stop-2100 >/dev/null 2>&1 \
  && echo "[4/4] 종료 스케줄 이미 있음 - 건너뜀" \
  || { aws scheduler create-schedule --region $R --name audit-lens-stop-2100 \
       --schedule-expression "cron(0 21 ? * MON-FRI *)" --schedule-expression-timezone "Asia/Seoul" \
       --flexible-time-window Mode=OFF \
       --target "{\"Arn\":\"arn:aws:scheduler:::aws-sdk:ec2:stopInstances\",\"RoleArn\":\"arn:aws:iam::$ACC:role/$ROLE\",\"Input\":\"{\\\"InstanceIds\\\":[\\\"$INST\\\"]}\"}" >/dev/null \
       && echo "[4/4] 평일 21:00 종료 스케줄 등록 완료"; }

# ③-2 구버전 종료 스케줄(18:10) 정리 — 남아 있으면 삭제(운영 시각이 21:00로 변경됨)
aws scheduler get-schedule --region $R --name audit-lens-stop-1810 >/dev/null 2>&1 \
  && { aws scheduler delete-schedule --region $R --name audit-lens-stop-1810 >/dev/null \
       && echo "[정리] 구버전 18:10 종료 스케줄 삭제 완료"; } \
  || echo "[정리] 구버전 18:10 스케줄 없음 - 건너뜀"
...
echo "위 표에 audit-lens-start-0900 / audit-lens-stop-2100 두 줄이 ENABLED로 보이면 성공입니다."
